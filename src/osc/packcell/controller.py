...
class OraclePackController:
    ...
    def run(self, max_steps=500):
        phases=[("reach", np.array([*self.object_pose[:2], .19]), self.gripper_open, 90),
                ("descend", self.object_pose, self.gripper_open, 80),
                ("close", self.object_pose, self.gripper_closed, 80),
        ...
        for name,target,grip,count in phases:
            for _ in range(count):
                ...
                if name == "reach" and np.linalg.norm(self.cell.ee_position()-self.object_pose) < .02: self.ladder.reach_success=True
                if name == "close" and len(set(h[0] for h in hits+[(b,a) for a,b in hits])) >= 2 and max(forces or [0]) > .01: self.ladder.opposing_contact=True
        ...
        return {"phases":asdict(self.ladder),"steps":steps,"success":self.ladder.stable_inside_box}
